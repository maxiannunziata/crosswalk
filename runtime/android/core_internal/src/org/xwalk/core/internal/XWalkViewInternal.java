...
/**
 * <p>
 * XWalkViewInternal represents an Android view for web apps/pages. Thus most of attributes for
 * Android view are valid for this class. Since it internally uses
 * <a href="http://developer.android.com/reference/android/view/SurfaceView.html">
 * android.view.SurfaceView</a> for rendering web pages by default, it can't be resized, rotated,
 * transformed and animated due to the limitations of SurfaceView. Alternatively, XWalkViewInternal
 * can be transformed and animated by using
 * <a href="http://developer.android.com/reference/android/view/TextureView.html"> TextureView</a>,
 * which is intentionally used to render web pages for animation support. Besides, XWalkViewInternal
 * won't be rendered if it's invisible.
 * </p>
 * <p>
 * Crosswalk provides two ways to choose TextureView or SurfaceView:
 * </p>
 * <ol>
 * <li>[To Be Deprecated] Set preference key {@link XWalkPreferencesInternal#ANIMATABLE_XWALK_VIEW}
 * to true to use TextureView, and vice versa. Notice that all XWalkViews share the same preference
 * value.</li>
 * <li>Application developer can set this attribute for a single XWalkView by XML without impact on
 * other XWalkViews, notice that in this case the value of
 * XWalkPreferencesInternal#ANIMATABLE_XWALK_VIEW is invaild for this XWalkView. See below steps for
 * detail:
 * <ul type="disc">
 ...
 * 
 * </li>
 * <li>Set xwalk attribute to true or false in the same xml as above. True for TextureView, false
 * for SurfaceView, and SurfaceView is the default.
 * 
 * <pre>
 ...
 * </li>
 * </ul>
 * There is debug message on logcat according to your "animatable" values: "XWalkContent:
 * CompositingSurfaceType is TextureView"</li>
 * </ol>
 * <p>
 ...
 * {@link XWalkUIClientInternal} for listening to the events related to resource loading and UI. By
 * default, Crosswalk has a default implementation. Callers can override them if needed.
 * </p>
 * <p>
 * <strong>Unlike WebView, you shouldn't use XWalkView directly. It must be accompanied with
 * {@link XWalkActivity} or {@link XWalkInitializer}. </strong>
 * </p>
 * <p>
 * For example:
 * </p>
 ...
 *
 *         &#64;Override
 *         public XWalkWebResourceResponseInternal shouldInterceptLoadRequest(
 ...
 *             // 1) createXWalkWebResourceResponse(String mimeType, String encoding, InputStream
 *             // data)
 *             // 2) createXWalkWebResourceResponse(String mimeType, String encoding, InputStream
 *             // data,
...
public class XWalkViewInternal extends android.widget.FrameLayout {
    ...
    @XWalkAPI
    public static final String CONTENT_SAVE_RESTORE_STATE_KEY = XWalkContent.SAVE_RESTORE_STATE_KEY;
    ...
    @XWalkAPI(preWrapperLines = {
            "        super(${param1}, ${param2});",
            "        if (isInEditMode()) return;", "        if (${param2} != null)",
    ...
    /**
     * Constructor for Crosswalk runtime. In shared mode, context isi different from activity. In
     * embedded mode, they're same.
     * 
     * @param context a Context object used to access application assets
    ...
    /**
     * Load a web page/app from a given base URL or a content. If url is null or empty and content
     * is null or empty, then this function will do nothing. If content is not null, load the web
     * page/app from the content. If content is not null and the url is not set, return
     * "about:blank" if calling {@link XWalkViewInternal#getUrl()}. If content is null, try to load
     * the content from the url. It supports URL schemes like 'http:', 'https:' and 'file:'. It can
     * also load files from Android assets, e.g. 'file:///android_asset/'.
     * 
     * @param url the url for web page/app.
     * @param content the content for the web page/app. Could be empty.
     * @deprecated This method is deprecated, please use {@link XWalkViewInternal#loadUrl},
    ...
    /**
     * Loads the given data into this WebView using a 'data' scheme URL. Note that JavaScript's same
     * origin policy means that script running in a page loaded using this method will be unable to
     * access content loaded using any scheme other than 'data', including 'http(s)'. To avoid this
     * restriction, use {@link XWalkViewInternal#loadDataWithBaseURL} with an appropriate base URL.
     * The encoding parameter specifies whether the data is base64 or URL encoded. If the data is
     * base64 encoded, the value of the encoding parameter must be 'base64'. For all other values of
     * the parameter, including null, it is assumed that the data uses ASCII encoding for octets
     * inside the range of safe URL characters and use the standard %xx hex encoding of URLs for
     * octets outside that range. For example, '#', '%', '\', '?' should be replaced by %23, %25,
     * %27, %3f respectively. The 'data' scheme URL formed by this method uses the default US-ASCII
     * charset. If you need need to set a different charset, you should form a 'data' scheme URL
     * which explicitly specifies a charset parameter in the mediatype portion of the URL and call
     * {@link XWalkViewInternal#loadUrl(String)} instead. Note that the charset obtained from the
     * mediatype portion of a data URL always overrides that specified in the HTML or XML document
     * itself.
     *
    ...
    /**
     * Loads the given data into this XWalkView, using baseUrl as the base URL for the content. The
     * base URL is used both to resolve relative URLs and when applying JavaScript's same origin
     * policy. The historyUrl is used for the history entry. Note that content specified in this way
     * can access local device files (via 'file' scheme URLs) only if baseUrl specifies a scheme
     * other than 'http', 'https', 'ftp', 'ftps', 'about' or 'javascript'. If the base URL uses the
     * data scheme, this method is equivalent to calling {@link XWalkViewInternal#loadData} and the
     * historyUrl is ignored, and the data will be treated as part of a data: URL. If the base URL
     * uses any other scheme, then the data will be loaded into the WebView as a plain string (i.e.
     * not part of a data URL) and any URL-encoded entities in the string will not be decoded. Note
     * that the baseUrl is sent in the 'Referer' HTTP header when requesting subresources (images,
     * etc.) of the page loaded using this method.
     *
     * @param baseUrl the URL to use as the page's base URL. If null defaults to 'about:blank'.
     * @param data a String of data in the given encoding
     * @param mimeType the MIMEType of the data, e.g. 'text/html'. If null, defaults to 'text/html'.
     * @param encoding the encoding of the data
     * @param historyUrl the URL to use as the history entry. If null defaults to 'about:blank'. If
    ...
    /**
     * Loads the given URL. It supports URL schemes like 'http:', 'https:' and 'file:'. It can also
     * load files from Android assets, e.g. 'file:///android_asset/'.
     *
     * @param url the url for web page/app.
     * @since 8.0
     */
     ...
     *
     * @param url the url for web page/app.
     * @param additionalHttpHeaders the additional headers to be used in the HTTP request for this
     *            URL, specified as a map from name to value. Note that if this map contains any of
     *            the headers that are set by default by this XWalkView, such as those controlling
     *            caching, accept types or the User-Agent, their values may be overriden by this
     *            XWalkView's defaults.
     * @since 8.0
     */
    @XWalkAPI
    ...
    /**
     * Load a web app from a given manifest.json file. If content is not null, load the
     * manifest.json from the content. If content is null, try to load the manifest.json from the
     * url. Note that url should not be null if the launched path defined in manifest.json is
     * relative. It supports URL schemes like 'http:', 'https:' and 'file:'. It can also load files
     * from Android assets, e.g. 'file:///android_asset/'.
     * 
    ...
    /**
     * Get the content height of current web page/app. NOTE: When page finished loading, it's
     * content size info may not be updated. Pls wait for a while for info updating.
     * 
    ...
    /**
     * Get the navigation history for current XWalkViewInternal. It's synchronized with this
     * XWalkViewInternal if any backward/forward and navigation operations.
     * 
    ...
    /**
     * Removes a previously injected Java object from this XWalkView. Note that the removal will not
     * be reflected in JavaScript until the page is next (re)loaded. See
     * {@link #addJavascriptInterface}.
     * 
    ...
    /**
     * Clear the resource cache. Note that the cache is per-application, so this will clear the
     * cache for all XWalkViews used.
     * 
    ...
    /**
     * Clear the resource cache. Note that it only clear the cache for the specified url.
     * 
    ...
    /**
     * Pause all layout, parsing and JavaScript timers for all XWalkViewInternal instances. It will
     * be called when the container Activity get paused. It can also be explicitly called to pause
     * timers. Note that it will globally impact all XWalkViewInternal instances, not limited to
    ...
    /**
     * Resume all layout, parsing and JavaScript timers for all XWalkViewInternal instances. It will
     * be called when the container Activity get resumed. It can also be explicitly called to resume
     * timers. Note that it will globally impact all XWalkViewInternal instances, not limited to
     * just this XWalkViewInternal.
     *
     * @since 1.0
    ...
    /**
     * Pause many other things except JavaScript timers inside rendering engine, like video player,
     * modal dialogs, etc. See {@link #pauseTimers} about pausing JavaScript timers. It will be
     * called when the container Activity get paused. It can also be explicitly called to pause
     * above things.
     * 
     * @since 1.0
     */
    ...
    /**
     * Resume video player, modal dialogs. Embedders are in charge of calling this during resuming
     * this activity if they call onHide. Typically it should be called when the activity for this
     * view is resumed. It will be called when the container Activity get resumed. It can also be
     * explicitly called to resume above things.
     * 
     * @since 1.0
     */
    @XWalkAPI
    ...
    /**
     * Release internal resources occupied by this XWalkViewInternal. It will be called when the
     * container Activity get destroyed. It can also be explicitly called to release resources.
     * 
     * @since 1.0
     */
    @XWalkAPI
    ...
    /**
     * Start another activity to get some data back. Here will start from the current XWalkView
     * Activity, but XWalkView embedder can override this API to do their additional job.
     * 
     * @deprecated This method is no longer supported
     */
    @Deprecated
    ...
    /**
     * Pass through activity result to XWalkViewInternal. Many internal facilities need this to
     * handle activity result like JavaScript dialog, Crosswalk extensions, etc. See
    ...
    /**
     * Pass through intents to XWalkViewInternal. Many internal facilities need this to receive the
     * intents like web notification. See
     * <a href="http://developer.android.com/reference/android/app/Activity.html">
    ...
    public boolean restoreState(Bundle inState) {
        if (mContent == null)
            return false;
        if (mContent.restoreState(inState) != null)
            return true;
        return false;
    }
    ...
    /**
     * Embedders use this to customize their handlers to events/callbacks related to resource
     * loading.
     * 
    ...
    /**
     * This method is used by Cordova for hacking. TODO(yongsheng): remove this and related test
     * cases?
     *
    ...
    /**
     * Enables remote debugging and returns the URL at which the dev tools server is listening for
     * commands.
     */
     ...
     * 
     * @return the web socket url to remote debug this xwalk view. null will be returned if remote
     ...
     * 
     * @param factor the zoom factor to apply. The zoom factor will be clamped to the XWalkView's
    ...
    /**
     * Create a new InputConnection for and InputMethod to interact with the view. The default
     * implementation returns the InputConnection created by ContentView
     * 
     ...
     * when XWalkPreferences.ANIMATABLE_XWALK_VIEW is false.
     * 
   ...
   @XWalkAPI
     ...
     * from this WebView's store. This is a poorly named method, but we keep it for historical
     * reasons.
     * 
     * @since 6.0
     */
     ...
     * @param maxOverScrollX the max distance the view could over scrolled on the horizontal
     *            orientation.
     * @param maxOverScrollY the max distance the view could over scrolled on the vertical
     *            orientation.
    ...
    @Override
    @XWalkAPI
    // NOTE: This API implementation is almost the same as the parent, but our onOverScrolled
    // couldn't
    ...
    /**
     * Compute the vertical offset the vertical scrollbar's thumb within the horizontal range.
     * 
    ...
    /**
     * Get the external extension manager for current XWalkView. Embedders could employ this manager
     * to load their own external extensions.
     * 
    ...
    /**
     * Clears the client certificate preferences stored in response to proceeding/cancelling client
     * cert requests.
     *
    ...
    /**
     * Gets the SSL certificate for the main top-level page or null if there is no certificate (the
     * site is not secure).
     * 
    ...
    /**
     * Finds all instances of find on the page and highlights them asynchronously. Notifies any
     * registered {@link XWalkFindListener}. Successive calls to this will cancel any pending
     * searches.
     *
    ...
    /**
     * Highlights and scrolls to the next match found by {@link #findAllAsync}, wrapping around page
     * boundaries as necessary. Notifies any registered {@link XWalkFindListener}. If
     * {@link #findAllAsync} has not been called yet, or if {@link #clearMatches} has been called
     * since the last find operation, this function does nothing.
     *
